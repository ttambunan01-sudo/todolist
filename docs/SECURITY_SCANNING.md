...
This document describes the security scanning implementation for the TodoList application, including Trivy vulnerability scanning and Dependabot automated dependency updates.
...
security-scan:
  ...
  steps:
    ...
    - name: Run Trivy vulnerability scanner
      ...
      with:
        image-ref: '${{ secrets.DOCKERHUB_USERNAME }}/todolist-backend:git-${{ github.sha }}'
        format: 'sarif'
...
- Gradle dependencies (Spring Framework, testing libraries)
- Docker base images
- GitHub Actions
...
Build & Test (build-and-test job)
        ↓
Quality Gate (sonarcloud-scan job)
        ↓
...
**Root Cause:**
Quality gate jobs (sonarcloud-scan) had conditions that excluded develop/staging branches:
```yaml
if: github.event_name == 'pull_request' || github.ref == 'refs/heads/main'
```

This caused a cascade effect:
1. sonarcloud-scan skips (condition not met)
2. docker-build-push skips (depends on sonarcloud-scan)
3. security-scan skips (depends on docker-build-push)

**Solution:**
Update sonarcloud-scan job condition to include all deployment branches:
```yaml
if: github.event_name == 'pull_request' || github.ref == 'refs/heads/main' || github.ref == 'refs/heads/develop' || github.ref == 'refs/heads/staging'
```

**Location:** `.github/workflows/backend-ci.yml` line 53
...
**Error Message:**
```
remote error: GET https://index.docker.io/v2/***/todolist-backend/manifests/git-abc123...:
...
**Location:** `.github/workflows/backend-ci.yml` line 138-142
...
- name: Extract metadata
  ...
  with:
    images: ${{ secrets.DOCKERHUB_USERNAME }}/todolist-backend
    tags: |
      type=raw,value=latest
      type=sha,prefix=git-,format=long
```

**Location:** `.github/workflows/backend-ci.yml` line 115
...
**Location:** `.github/workflows/backend-ci.yml` line 133-136
...
4. **Maintenance**
   - Keep Trivy action up to date
   - Review Dependabot configuration quarterly
   - Adjust severity thresholds as needed

## Related Documentation

- Frontend Security: `/Users/tumpaltambunan/todolistreact/todolist-app/docs/SECURITY_SCANNING.md`
